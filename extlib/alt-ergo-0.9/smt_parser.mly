...
formula:
| TRUE                                                { mk_form True }
| LP TRUE annots RP                                   { mk_form True }
| FALSE                                               { mk_form False }
| LP FALSE annots RP                                  { mk_form False }
| FVAR                                                { mk_form (Fvar $1) }
| LP FVAR annots RP                                   { mk_form (Fvar $2) }
| ID                                                  { mk_form (Pred($1,[]) )}
| LP symb terms opt_annots RP                         { mk_form (Pred($2,List.rev $3)) }
| LP DISTINCT terms opt_annots RP                     { mk_form (Distinct (List.rev $3))}
| LP EQUALS terms opt_annots RP                       { mk_form (Equals (List.rev $3))}
| LP NOT formula opt_annots RP                        { mk_form (Not $3) }
| LP IMPLIES formula formula opt_annots RP            { mk_form (Implies($3,$4)) }
| LP IFTHENELSE formula formula formula opt_annots RP { mk_form (Fite($3,$4,$5)) }
| LP AND formulas opt_annots RP                       { mk_form (And $3) }
| LP OR  formulas opt_annots RP                       { mk_form (Or $3) }
| LP XOR formulas opt_annots RP                       { mk_form (Xor $3) }
| LP IFF formulas opt_annots RP                       { mk_form (Iff $3) }
| LP FORALL binders formula opt_annots RP          { mk_form (Forall(List.rev $3,$4)) }
| LP EXISTS binders formula opt_annots RP          { mk_form (Exists(List.rev $3,$4)) }
| LP LET LP VAR term RP formula opt_annots RP      { mk_form (Let($4,mk_term $5,$7)) }
| LP FLET LP FVAR formula RP formula opt_annots RP { mk_form (Flet($4,$5,$7)) }
;
