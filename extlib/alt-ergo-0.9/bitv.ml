...
module Make(X : ALIEN) = struct
  ...
  let is_mine_hs t = 
    match T.view t with
	{T.f = (Sy.Bitv _ | Sy.Binop Sy.At) } -> true
      | {T.f = s} -> Sy.equal s Sy.shat

  let is_mine_type _ = assert false

  let normal_form x = x

  let embedding r = assert false
  ...
  module Canonizer = struct
    ...
    let make t =
      let rec make_rec t' = match T.view t' with
	| {T.f = Sy.Bitv s } -> string_to_bitv s
	| {T.f = Sy.Binop Sy.At ; xs = [t1;t2] ; ty = Ty.Tbitv n} ->
	    { bv = I_Comp (make_rec t1,make_rec t2) ; sz = n }
	| {T.f = s; xs = [t1;ti;tj] ; ty = Ty.Tbitv n} 
	    when Sy.equal s Sy.shat ->
	    begin 
	      match T.view ti , T.view tj with
		| { T.f = Sy.Int i } , { T.f = Sy.Int j } -> 
		    let i = int_of_string i.node in
		    let j = int_of_string j.node in
		    { sz = j - i + 1 ; bv = I_Ext (make_rec t1,i,j)} 
		| _ -> assert false
	    end
	| {T.ty = Ty.Tbitv n} -> 
	    {bv = I_Other (Alien (X.make t')) ; sz = n}
	| _ -> assert false
      in sigma (make_rec t)   
  end
  ...
  let make t = is_mine (Canonizer.make t)
  ...
  let size_of r = 
    match X.type_infos r with Ty.Tbitv i -> i | _ -> 
