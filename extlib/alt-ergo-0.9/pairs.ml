  ...
  let make t = 
	...
	| {Term.f = Symbols.Name (f,_); xs = l; ty=ty} ->
	    begin
	      if is_pair f then
		match l with 
		  | [lc; rc] -> Pair (make_rec lc, make_rec rc,ty)
		  | _ -> assert false
	...
	| {Term.ty=ty} -> Other (X.make t,ty)
    in 
    is_mine (make_rec t)

  let embedding r = Other(r,X.type_infos r)
...
(* elements of type t, implemented as a set of lists. *)
  ...
  let is_mine_hs t = 
    match (Term.view t) with
      | {Term.f = Symbols.Name (f,_)} -> is_fst f || is_snd f || is_pair f
      | _ -> false
  ...
  let type_snd t = 
    match type_infos t with 
	Ty.Text(([_;ty] | [ty]),_) -> ty | _ -> assert false

  let type_fst t = 
    match type_infos t with Ty.Text(([ty;_]|[ty]),_) -> ty  
      | _ -> assert false
...
(** [find_new_eqs uf] tries to find new equalities between elements
  ...
  let solve r1 r2 = 
	...
	Some t1 -> t1 | None -> Other(r1,X.type_infos r1) in
    let t2 = match X.extract r2 with 
	Some t2 -> t2 | None -> Other(r2,X.type_infos r2) in
    ...
    let f tag =
      List.fold_left
	(fun acc t -> 
	   match UF.find_new_mate ufsat (Other(t,X.type_infos t)) tag with
...
(* 	      sols (l2,UF.Left) (l1,UF.Right) *)
    ...
    let res = match t1,t2 with
	Other _ , Other _ -> assert false
      | Other (p,_) , v -> [p,X.embed v]
      | v , Other (p,_) -> [p,X.embed v]
      | _ , _ -> 
	  let sols (l,t) (l',t') =
	    match f t l with [] -> f t' l' | r -> r
	  in
...
(* 	      sols (l2,UF.Left) (l1,UF.Right) *)
  ...
  struct
    ...
    let empty _ = ()
    let assume _ _ _ _ = ((),Literal.SetEq.empty)
    let query _ _ _ _ = assert false
  end
end
