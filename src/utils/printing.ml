...
let make_json_source_pos node_id text =
  ...
  let make_json_spec_source_pos pos text =
    let (sline, eline, spos, epos) = match pos with
      | Some (sline, eline, spos, epos) -> (sline, eline, spos, epos)
      | None -> (-1, -1, -1, -1) in
    Printf.printf "\njson {\"error_pos\": {\"sline\": \"%d\", \"eline\": \"%d\", \"spos\": \"%d\", \"epos\": \"%d\"}, \"error_text\": \"%s\"}\n"
      sline eline spos epos text
...
let eclipse_print node_id error_text =
