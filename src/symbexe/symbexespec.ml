...
open Sepprover (* TODO(rgrig): remove before commit, so that the uses of the prover are clear *)
open Spec

let conjunction_excep_convert excep_post f1 =
  ClassMap.map (fun post -> conjoin post f1) excep_post
...
let implication_excep logic excep1 excep2 = 
  try 
    ClassMap.iter (
    fun exname form -> 
      if implies logic form (ClassMap.find exname excep2) 
      then ()
