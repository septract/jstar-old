...
(*  mutable nd_id:int; (* unique identifier for node *) *)
  ...
  mutable nd_stmt : stmt_core;
  mutable nd_succs : node_core list;
  mutable nd_preds : node_core list;
  mutable nd_method : method_cfg_info option 
}
and method_cfg_info = {   
  pd_md : methdec; 
  pd_start_node : node_core; 
  pd_exit_node : node_core;
}

type id_node_hashtbl = (int, node_core) Hashtbl.t
...
let node_list : node_core list ref = ref []
...
let print_node n =
  Printf.printf "\n Node statement sid=%i  skind=%s" n.nd_stmt.sid (statement_core2str n.nd_stmt.skind);
...
let icfg_create_method_cfg_infos (lmd: Global_types.methdec list) : unit = 
...
let rec cfgStmts (mdesc: method_cfg_info) (ss: stmt_core list) 
    (next:stmt_core option) (break:stmt_core option) (cont:stmt_core option) =
  match ss with
    | [] -> ()
...
and cfgBlock (mdesc: method_cfg_info) (bstmts: stmt_core list) 
    (next:stmt_core option) (break:stmt_core option) (cont:stmt_core option) = 
...
and cfgStmt (minfo: method_cfg_info) (s: stmt_core) 
    (next:stmt_core option) (break:stmt_core option) (cont:stmt_core option) =
(*    incr num_stmts;
    s.sid <- !num_stmts;
*)
...
let compute_start_stmts md = 
  let start_stmts=List.filter (fun s -> s.preds = [])  md.bstmts in
    if debug then begin
      ...
      let ls=list2str statement_core2str start_statements "\n"in
      if cfg_debug() then Printf.printf "\n start_node=[ %s ]\n" ls
    end;
    start_stmts  
...
let compute_exit_stmts md = 
  let exit_stmts=List.filter (fun s -> s.succs = [])  md.bstmts in
    if debug then begin
      ...
      let ls=list2str statement_core2str exit_statements "\n"in
      if cfg_debug() then Printf.printf "\n exit_node=[ %s ]\n" ls
    end;
    exit_stmts  
