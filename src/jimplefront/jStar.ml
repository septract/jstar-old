...
let get_javac_cmd cp sp jf =
  Array.of_list &
    ...
    (if Path.is_empty sp then ["-sourcepath"; Path.to_string sp] else []) @
    jf
...
let _ = object 
  inherit [StringSet.t] Jimple_visitors.evaluator StringSet.empty
...
let _ = object
  inherit Jimple_visitors.mapper
  method map_class_name = function
    | JPT.Quoted_clname cn -> JPT.Quoted_clname "foo"
    | JPT.Identifier_clname cn -> JPT.Identifier_clname "foo"
    | JPT.Full_identifier_clname cn -> JPT.Full_identifier_clname "foo"
end

(* Turns [X = new T; specialinvoke X.<T: void<init>()>()]
   into  [X = new T; virtualinvoke X.<T: void<init>()>()].
   Note that I assume new is immediately followed by specialinvoke.
   This seems to be guaranteed by
   [src/soot/jimple/toolkits/base/JimpleConstructorFolder.java]
   in Soot. *)
let virtualinvoke_of_specialinvoke j =
  failwith "not implemented. see run_parser.ml:77"

let preprocess_jimple j =
  virtualinvoke_of_specialinvoke j

let load_jimple cn =
  let fn = jimple_dir / cn ^ ".jimple" in
  let ch = open_in fn in
  let lexbuf = Lexing.from_channel ch in
  let j = Jparser.file Jlexer.token lexbuf in
  close_in_noerr ch; 
  preprocess_jimple j

let light_load_jimple _ = failwith "light_load_jimple not implemented"
let mentioned_classes _ = failwith "mentioned_classes not implemented"
let load_spec _ = failwith "load_spec not implemented"
let load_logic _ = failwith "load_logic not implemented"
let load_abstraction _ = failwith "load_abstraction not implemented"
let preprocess _ _ _ _ = failwith "preprocess not implemented"
let symbolically_execute _ _ _ _ _ _ = 
  failwith "symbolically_execute not implemented"

let verify_class cn =
  let cj = load_jimple cn in
  let deps = mentioned_classes cj in
  let cns = cn :: deps in
  let code = cj :: List.map light_load_jimple deps in
  let spec = load_spec cns in
  let logic = load_logic cns in
  let abs = load_abstraction cns in
  let static_spec, dynamic_spec, logic, abs = preprocess code spec logic abs in
  symbolically_execute cn code static_spec dynamic_spec logic abs
