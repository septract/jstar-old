...
\]
We call $F_1$ the \emph{assumed formula}, $F_2$ the \emph{goal formula}, and $\Sigma$ the \emph{subtracted (spatial)
formula}. 
Rules are \emph{implicitly framed} -- the semantics of the rule includes the contexts $H_s$, $H_a$ and $H_g$. This means rules can be applied inside large context heaps. 

%A sequent in the rule above is of the form
%\[
%\Sigma * H_s \; \mid \; F_1 * H_a \; \vdash \; F_2 * H_g
%\]

The semantics of a judgement is:
\[
