...
%
\begin{verbatim}
 define Val(x,content=y;oldcon=z) as  Val$Cell(x, {content=y}) * x.<Recell: int bak>|-> z 
\end{verbatim}
%\begin{lstlisting}
...
the superclass \J~Cell~ and the additional \J~oldcon~ field has value
...
that the fields have different  values.  The clause {\tt without} is useful to avoid the rule to be applied
