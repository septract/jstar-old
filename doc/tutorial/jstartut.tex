...
\begin{verbatim}
 define Val(x,content=y;oldcon=z) as  
     Val$Cell(x, {content=y}) * x.<Recell: int bak>|-> z 
\end{verbatim}
%\begin{lstlisting}
...
\begin{verbatim}
Val(x, content=y) as Val$Cell(x,content= y ) * x.<Recell: int bak>|-> _z
\end{verbatim}
%
...
\dinocomment{is this paragraph correct?}

\paragraph{Axioms.} 
\dinocomment{I'm not sure of this paragraph....}
Rules with an empty premise are axioms. For example the rule {\tt field\_not\_null} 
...
if
  | |-
\end{verbatim}
\dinocomment{Why is the field predicate in the subtracted formula side?}
