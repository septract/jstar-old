...
parameter, \J~oldcon~, we must also provide a meaning to the property
with only the \J~content~ parameter.  In effect, we have width
...
if
  field(?x,?f,?y) | |- ?y=?t 
\end{verbatim}
It states that if we have a field {\tt ?f} for the same object {\tt ?x} on both sides of an implication, add the 
 field to the subtracted formula, and require for proof obligation that their values {\tt ?y} and {\tt ?t} are the same. Here the clause {\tt without} prevents the firing of the rule if the inequality {\tt ?y!=?t} is
present in one of the formulae of the conclusion, or in other words,  when the formula already says
that the fields have different  values.  The clause {\tt without} is useful to avoid the rule being applied
an infinite amount of times.



\paragraph{Contradictions.} 
Rules with an empty premise can be used  to force the termination of a proof
search in case we reached a contradictory statement.
%
For example consider the rule {\tt field\_nil\_contradiction} below. It fires when in a proof search 
we have reached a point where a field for {\tt nil()} is assumed to exist. Clearly, in that case
 we have found a contradiction, and {\tt field\_nil\_contradiction}  will stop the proof search.
\begin{verbatim}
rule field_nil_contradiction :
 field(nil(),?f,?z) | |-
if
\end{verbatim}\jStarQED
...
{H*ls(y,\_x)*ls(\_x,\nil) \leadsto H*ls(y,\nil)}
{\_x \notin Var(H) \cup \{ y \} }
\] This rule abstracts away from the useless quantified variable {\tt \_x} and, therefore, it reduces the original heap involving two list-segment predicates into a formula with only such predicate.\jStarQED
